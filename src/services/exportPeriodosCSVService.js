...
const estiloEncabezado = {
  ...
  border: {
    bottom: { style: 'medium', color: { argb: COLORES.accentBlue } }
  }
}
...
const estiloTotales = {
  ...
  alignment: { horizontal: 'center', vertical: 'middle' },
  border: {
    top: { style: 'medium', color: { argb: COLORES.accentGreen } }
  }
}
...
/**
 * Exporta cierres por período en XLSX profesional
 */
export const exportarCierresPorPeriodoCSV = async (periodoRaw) => {
  ...
  wb.creator = 'GymControl - La Roca'
  wb.created = new Date()

  const ws = wb.addWorksheet('Consolidado de Cierres', {
  ...
  titleCell.value = 'GYMCONTROL — CONSOLIDADO DE CIERRES'
  titleCell.font = { bold: true, size: 14, color: { argb: COLORES.accentBlue }, name: 'Arial' }
  titleCell.alignment = { horizontal: 'center' }

  ws.mergeCells('A2:D2')
  const subtitleCell = ws.getCell('A2')
  subtitleCell.value = `Gimnasio La Roca  |  Período: ${desde} al ${hasta}`
  ...
  headerRow.eachCell((cell) => {
    Object.assign(cell, { font: estiloEncabezado.font, fill: estiloEncabezado.fill, alignment: estiloEncabezado.alignment, border: estiloEncabezado.border })
  })
  headerRow.height = 28
  ...
  totRow.eachCell((cell) => {
    Object.assign(cell, { font: estiloTotales.font, fill: estiloTotales.fill, alignment: estiloTotales.alignment, border: estiloTotales.border })
  })
  totRow.height = 30
  ...
  await descargarWorkbook(wb, `cierres_${desde}_a_${hasta}.xlsx`)
}

/**
 * Exporta lista de miembros en XLSX
 */
export const exportarMiembrosXLSX = async () => {
  const { data, error } = await supabase
    .from('socios')
  ...
  title.value = 'GYMCONTROL — LISTA DE MIEMBROS'
  title.font = { bold: true, size: 14, color: { argb: COLORES.accentBlue }, name: 'Arial' }
  title.alignment = { horizontal: 'center' }

  ws.mergeCells('A2:F2')
  const sub = ws.getCell('A2')
  sub.value = `Gimnasio La Roca  |  ${data.length} miembros activos  |  ${new Date().toLocaleDateString('es-VE', { timeZone: 'America/Caracas' })}`
  ...
  await descargarWorkbook(wb, `miembros_${new Date().toISOString().split('T')[0]}.xlsx`)
}

/**
 * Exporta pagos del día en XLSX
 */
export const exportarPagosDelDiaXLSX = async () => {
  ...
  ws.mergeCells('A1:F1')
  const title = ws.getCell('A1')
  title.value = `GYMCONTROL — PAGOS DEL DÍA ${hoy}`
  title.font = { bold: true, size: 14, color: { argb: COLORES.accentBlue }, name: 'Arial' }
  title.alignment = { horizontal: 'center' }

  ws.mergeCells('A2:F2')
  const sub = ws.getCell('A2')
  sub.value = `Gimnasio La Roca  |  ${data.length} pagos registrados`
  sub.font = { size: 10, color: { argb: '6B7280' }, name: 'Arial' }
  sub.alignment = { horizontal: 'center' }
  ...
  await descargarWorkbook(wb, `pagos_${hoy}.xlsx`)
}

/**
 * Exporta asistencias del día en XLSX
 */
export const exportarAsistenciasDelDiaXLSX = async () => {
  const hoy = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Caracas' })
  const inicio = new Date(hoy + 'T00:00:00').toISOString()
  const fin = new Date(hoy + 'T23:59:59').toISOString()
  ...
  ws.mergeCells('A1:D1')
  const title = ws.getCell('A1')
  title.value = `GYMCONTROL — ASISTENCIAS DEL DÍA ${hoy}`
  title.font = { bold: true, size: 14, color: { argb: COLORES.accentBlue }, name: 'Arial' }
  title.alignment = { horizontal: 'center' }

  ws.mergeCells('A2:D2')
  const sub = ws.getCell('A2')
  sub.value = `Gimnasio La Roca  |  ${unicos.length} entradas registradas`
  sub.font = { size: 10, color: { argb: '6B7280' }, name: 'Arial' }
  sub.alignment = { horizontal: 'center' }
  ...
  await descargarWorkbook(wb, `asistencias_${hoy}.xlsx`)
}