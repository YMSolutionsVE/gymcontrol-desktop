...
var calcRango = function(id) {
  var h = new Date()
  var hasta = hoyStr()
  var d
  switch (id) {
    case 'hoy':   return { desde: hasta, hasta: hasta }
  ...
  return { desde: d.toISOString().split('T')[0], hasta: hasta }
}

var calcRangoAnterior = function(desde, hasta) {
  var d1 = new Date(desde + 'T00:00:00')
  var d2 = new Date(hasta + 'T00:00:00')
  var dias = Math.round((d2 - d1) / (1000 * 60 * 60 * 24))
  var prevHasta = new Date(d1); prevHasta.setDate(prevHasta.getDate() - 1)
  var prevDesde = new Date(prevHasta); prevDesde.setDate(prevDesde.getDate() - dias)
...
var formatPeriodoLabel = function(id, desde, hasta) {
  switch (id) {
...
export default function Reportes() {
  ...
  var gymNombre = authHook.gymNombre

  var [periodoActivo, setPeriodoActivo] = useState('7d')
  var initRango = calcRango('7d')
  var [desde, setDesde] = useState(initRango.desde)
  var [hasta, setHasta] = useState(initRango.hasta)

  var [compararPeriodo, setCompararPeriodo] = useState(false)
  var [metricasAnteriores, setMetricasAnteriores] = useState(null)
  var [datosIngresosAnt, setDatosIngresosAnt] = useState([])
  var [datosAsistenciasAnt, setDatosAsistenciasAnt] = useState([])

  var [actividad, setActividad] = useState([])
  var [loading, setLoading] = useState(false)
  var [mostrarCierre, setMostrarCierre] = useState(false)
  var [resumenHoy, setResumenHoy] = useState(null)
  var [cierreConsultado, setCierreConsultado] = useState(null)
  var [cerrando, setCerrando] = useState(false)
  ...
  var cargarComparacion = useCallback(async function(d, h) {
    var prev = calcRangoAnterior(d, h)
    var results = await Promise.all([
    ...
    if (results[0].success) setDatosIngresosAnt(results[0].data)
    if (results[1].success) setDatosAsistenciasAnt(results[1].data)
    if (results[2].success) setMetricasAnteriores(results[2].data)
  }, [gymId])
  ...
  var handleCustomDesde = function(e) { setDesde(e.target.value); setPeriodoActivo('custom') }
  var handleCustomHasta = function(e) { setHasta(e.target.value); setPeriodoActivo('custom') }

  var abrirCierreHoy = async function() {
    var data = await obtenerResumenHoy(gymId)
    if (!data) return
    setResumenHoy(data); setCierreConsultado(null); setMostrarCierre(true)
  }

  var confirmarCierre = async function() {
    setCerrando(true)
    var res = await cerrarDia(user.id, resumenHoy, gymId)
    if (res.success) {
  ...
  var consultarCierre = async function(fecha) {
    var res = await obtenerCierrePorFecha(fecha, gymId)
  ...
  var calcVariation = function(current, previous) {
    if (!previous || previous === 0) return null
    return ((current - previous) / previous * 100).toFixed(1)
  }

  var cierreTotales = cierre && cierre.detalle_pagos
    ? calcularTotalesMultiMoneda(cierre.detalle_pagos)
    : { totalUsd: 0, totalEur: 0, totalBs: 0 }

  var tieneUsd = cierreTotales.totalUsd > 0
  var tieneEur = cierreTotales.totalEur > 0

  var totalDescuentosCierre = 0
  var pagosConDescuento = 0
  if (cierre && cierre.detalle_pagos && cierre.detalle_pagos.length > 0) {
    cierre.detalle_pagos.forEach(function(p) {
      var desc = Number(p.descuento || 0)
      if (desc > 0) {
        totalDescuentosCierre += desc
        pagosConDescuento++
      }
    })
  }
  var hayDescuentosCierre = totalDescuentosCierre > 0

  var cierreDetalleTieneDescuentos = cierre && cierre.detalle_pagos
    ? cierre.detalle_pagos.some(function(p) { return Number(p.descuento || 0) > 0 })
    : false
    ...
    <div className="p-8 max-w-[1200px]">
      ...
      <div className="bg-[#0D1117] rounded-xl border border-white/[0.06] p-4 mb-6">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <button
            onClick={function() { setCompararPeriodo(!compararPeriodo) }}
      ...
      <MetricasResumen
        ...
        totalDescuentos={metricas.totalDescuentos || 0}
        cantidadDescuentos={metricas.cantidadDescuentos || 0}
        descuentosPorMoneda={metricas.descuentosPorMoneda || {}}
      ...
      <div className="bg-[#0D1117] rounded-xl border border-white/[0.06] overflow-hidden mb-8">
        <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between">
          ...
          <span className="text-gray-600 text-[10px] tabular-nums">
            {actividad.length + ' día' + (actividad.length !== 1 ? 's' : '')}
          </span>
        </div>
        <div className="divide-y divide-white/[0.04] max-h-[300px] overflow-y-auto">
          {!loading && actividad.map(function(dia) {
            return (
              <div
                key={dia.fecha}
                onClick={function() { consultarCierre(dia.fecha) }}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#111827] border border-white/[0.08] w-full max-w-5xl rounded-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={function() { exportarCierreCajaPDF(cierre, cierre.fecha, gymNombre) }}
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className={'grid gap-4 mb-6 ' + (tieneUsd && tieneEur && hayDescuentosCierre ? 'grid-cols-5' : tieneUsd && tieneEur ? 'grid-cols-4' : hayDescuentosCierre ? 'grid-cols-4' : 'grid-cols-3')}>
                {tieneUsd && (
                ...
                {hayDescuentosCierre && (
                  <div className="relative overflow-hidden bg-[#0D1117] border border-white/[0.06] rounded-xl p-4">
                    <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-gradient-to-b from-amber-500 to-amber-600 rounded-l-xl" />
                    <p className="text-gray-400 text-xs font-medium mb-1 pl-2">Descuentos</p>
                    <p className="text-amber-400 font-bold text-2xl pl-2 tabular-nums">{pagosConDescuento}</p>
                    <p className="text-amber-400/60 text-[11px] pl-2 mt-0.5">
                      {'Total: ' + formatMoney(totalDescuentosCierre) + ' (divisa)'}
                    </p>
                  </div>
                )}
              </div>
              ...
              {cierre.detalle_metodos && Object.keys(cierre.detalle_metodos).length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {Object.entries(cierre.detalle_metodos).map(function(entry) {
                      return (
                        <div key={entry[0]} className="bg-[#0D1117] border border-white/[0.06] rounded-lg px-4 py-3">
                          <p className="text-gray-400 text-xs capitalize mb-1">{entry[0].replace('_', ' ')}</p>
                          <p className="text-emerald-400 font-bold tabular-nums">{'Bs ' + formatMoney(Number(entry[1]))}</p>
                        </div>
                      )
                    })}
                  </div>
              ...
              {cierre.detalle_pagos && cierre.detalle_pagos.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[10px] text-gray-500 font-semibold tracking-[0.15em] uppercase">Detalle de pagos</p>
                    {hayDescuentosCierre && (
                      <span
                        className="text-[10px] px-2 py-0.5 rounded-full font-medium"
                        style={{ background: 'rgba(245,158,11,0.1)', color: '#fbbf24', border: '1px solid rgba(245,158,11,0.2)' }}
                      >
                        {pagosConDescuento + ' con descuento'}
                      </span>
                    )}
                  </div>
                  <div className="bg-[#0D1117] border border-white/[0.06] rounded-xl overflow-hidden">
                    <div className={'grid gap-2 px-4 py-2.5 border-b border-white/[0.06] text-[10px] text-gray-500 font-semibold uppercase tracking-wider ' + (cierreDetalleTieneDescuentos ? 'grid-cols-7' : 'grid-cols-6')}>
                      <span>Miembro</span>
                      <span>Cédula</span>
                      <span>Método</span>
                      <span>Monto</span>
                      {cierreDetalleTieneDescuentos && <span>Descuento</span>}
                      <span>Bs</span>
                      <span>Ref.</span>
                    </div>
                    {cierre.detalle_pagos.map(function(p, i) {
                      var moneda = (p.moneda_divisa || 'USD').toUpperCase()
                      var montoPrincipal = Number(p.monto_divisa || p.monto_usd || 0)
                      var descPago = Number(p.descuento || 0)
                      var tieneDesc = descPago > 0

                      return (
                        <div key={i} className={'grid gap-2 px-4 py-2.5 border-b border-white/[0.03] text-sm text-gray-300 hover:bg-white/[0.02] ' + (cierreDetalleTieneDescuentos ? 'grid-cols-7' : 'grid-cols-6')}>
                          <span className="truncate">{(p.socios && p.socios.nombre) || '-'}</span>
                          <span className="text-gray-400">{(p.socios && p.socios.cedula) || '-'}</span>
                          <span className="capitalize text-gray-400">{(p.metodo || '').replace('_', ' ')}</span>
                          <div className="tabular-nums">
                            <span style={{ color: moneda === 'EUR' ? '#60a5fa' : '#34d399' }}>
                              {getCurrencyBadge(moneda) + formatMoney(montoPrincipal)}
                            </span>
                            {tieneDesc && (
                              <span className="text-gray-600 text-[11px] line-through ml-1">
                                {formatMoney(montoPrincipal + descPago)}
                              </span>
                            )}
                          </div>
                          {cierreDetalleTieneDescuentos && (
                            <div>
                              {tieneDesc ? (
                                <div>
                                  <span className="text-amber-400 text-xs font-medium tabular-nums">
                                    {'-' + getCurrencyBadge(moneda) + formatMoney(descPago)}
                                  </span>
                                  {p.motivo_descuento && (
                                    <p className="text-amber-400/50 text-[10px] truncate mt-0.5" title={p.motivo_descuento}>
                                      {p.motivo_descuento}
                                    </p>
                                  )}
                                </div>
