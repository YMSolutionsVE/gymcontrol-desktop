    ...
    <div className="p-8 max-w-[1200px]">
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          {/* BCV inline */}
          <div className="flex items-center gap-2 bg-[#0D1117] border border-white/[0.06] rounded-xl px-4 py-2.5">
      ...
      {(stats.porVencer > 0 || stats.vencidos > 0) && (
        <div className="space-y-2 mb-6">
          ...
          {stats.vencidos > 0 && (
            <div className="flex items-center gap-3 bg-red-500/[0.06] border border-red-500/15 rounded-xl px-4 py-3">
      ...
      {/* Stats grid - row 1: members */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        <StatCard
          title="Miembros activos" value={stats.activos}
          subtitle={`${stats.totalSocios} registrados`}
          color="blue" icon="members"
        />
        <StatCard
          title="Vencidos" value={stats.vencidos}
          color="red" icon="alert"
        />
        <StatCard
          title="Por vencer" value={stats.porVencer}
          subtitle="Próximos 3 días"
          color="yellow" icon="clock"
        />
        <StatCard
          title="Entradas hoy" value={stats.asistenciasHoy}
          color="purple" icon="entry"
        />
      </div>

      {/* Stats grid - row 2: revenue (larger) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <StatCard
          title="Ingresos hoy"
          value={`$${stats.ingresosHoy.toFixed(2)}`}
          subtitle={`Bs. ${(stats.ingresosHoy * tasaBcv).toFixed(2)}`}
          color="green" icon="dollar" size="large"
        />
        <StatCard
          title="Ingresos del mes"
          value={`$${stats.ingresosMes.toFixed(2)}`}
          subtitle={`Bs. ${(stats.ingresosMes * tasaBcv).toFixed(2)}`}
          color="green" icon="dollar" size="large"
        />
      </div>
    </div>
