...
export default function PagoForm({ socio, onComplete, onCancel }) {
  ...
  useEffect(() => {
    if (planes.length > 0 && socio.plan_id) {
      const planActual = planes.find((plan) => plan.id === socio.plan_id)
      if (planActual) {
  ...
  useEffect(() => {
    ...
    const monedaPlan = getPlanCurrency(selectedPlan)
    setMontoBs(convertForeignToBs(form.monto_referencia, monedaPlan, config))
  }, [form.monto_referencia, selectedPlan, config])
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.es_cortesia) {
      ...
      const monto = parseFloat(form.monto_referencia)
      ...
      const monedaPlan = getPlanCurrency(selectedPlan)
      const tasaRequerida = getRateForCurrency(config, monedaPlan)

      if (!tasaRequerida || tasaRequerida <= 0) {
        return setError(`Configura la tasa ${getCurrencySymbol(monedaPlan)} antes de registrar pagos`)
      }
    ...
    const monedaPlan = getPlanCurrency(selectedPlan)
    const montoDivisa = parseFloat(form.monto_referencia) || 0
    const tasaUsada = getRateForCurrency(config, monedaPlan)

    const result = await registrarPago(gymId, {
      socio_id: socio.id,
      monto_usd: form.es_cortesia ? 0 : (monedaPlan === 'USD' ? montoDivisa : 0),
      monto_bs: form.es_cortesia ? 0 : montoBs,
      moneda_divisa: form.es_cortesia ? null : monedaPlan,
      monto_divisa: form.es_cortesia ? 0 : montoDivisa,
                ...
                >
                  ...
                  {planes.map((plan) => (
                    <option key={plan.id} value={plan.id}>
                      {plan.nombre} — {getCurrencyBadge(getPlanCurrency(plan))}{parseFloat(plan.precio_usd).toFixed(2)} ({plan.tipo === 'sesiones' ? `${plan.cantidad_sesiones} sesiones` : `${plan.duracion_dias} días`})
                    </option>
                  ))}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-gray-500">
                          {getCurrencyLabel(monedaPlan)} · {selectedPlan.tipo === 'sesiones' ? `${selectedPlan.cantidad_sesiones} sesiones` : `${selectedPlan.duracion_dias} días`}
              ...
              <div>
                ...
                <div>
                  <label className="block text-gray-400 text-xs font-medium mb-1.5">
                    Monto en {getCurrencyLabel(monedaPlan)} ({getCurrencyBadge(monedaPlan)})
                  </label>
                  <input
                    type="number"
                    step="0.01"
                    value={form.monto_referencia}
                    onChange={(e) => handleChange('monto_referencia', e.target.value)}
                    className={inputClass}
                    disabled={saving}
                    placeholder="0.00"
                  />
                  <p className="text-gray-500 text-xs mt-1.5 tabular-nums">
                    Equivalente: <span className="text-gray-400">Bs. {formatMoney(montoBs)}</span> · Tasa {getCurrencySymbol(monedaPlan)} vigente
                  </p>
                </div>
              </div>
